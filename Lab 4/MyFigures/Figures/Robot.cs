...
namespace MyFigures
{
    public class Robot : Figure
    {
        ...
        public Robot(string name, int x, int y, int width, int height)
        {
            if (x < 0 || y < 0 || x + width > pictureBox.Width || y + height > pictureBox.Height)
            {
                MessageBox.Show("Фигура должна полностью помещаться на холст!");
        ...
        public void change_all(int x, int y, int width, int height)
        {
            if (x < 0 || y < 0 || x + width > pictureBox.Width || y + height > pictureBox.Height)
            {
                MessageBox.Show("Фигура должна полностью помещаться на холст!");
        ...
        public void DeleteF(Robot figure, bool flag)
        {
            Graphics g = Graphics.FromImage(bitmap);
            g.FillRectangle(Brushes.White, x, y, width, height);
            FList.figureList.Remove(figure);
            ClearMap();
            foreach (Robot f in FList.figureList)
            {
                f.Draw_with_name();
            }
            if (flag != true)
